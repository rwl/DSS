...
public class GeneratorImpl extends PowerConversionElementImpl implements Generator {
		...
		if (getCircuit().getSolution().isDynamicModel() || getCircuit().getSolution().isHarmonicModel()) {
			...
			for (int i = 0; i < getNPhases(); i++) {
				...
				case LN:
					...
					dComplexMatrix2D.setQuick(getNConds(), getNConds(), dComplexMatrix2D.getRealPart().getQuick(getNConds(), getNConds()) + Y[0],
				...
				case DELTA:
					dComplexMatrix2D.setQuick(i, i, Y);
					dComplexMatrix2D.setQuick(i, i, dComplexMatrix2D.getRealPart().getQuick(i, i) + Y[0], dComplexMatrix2D.getImaginaryPart().getQuick(i, i) + Y[1]);
					for (int j = 0; j < i - 1; j++) {
			...
			case WYE:
				...
				for (int i = 0; i < getNPhases(); i++) {
					dComplexMatrix2D.setQuick(i, i, Y);
					dComplexMatrix2D.setQuick(getNConds(), getNConds(), DComplexFunctions.plus.apply(dComplexMatrix2D.getQuick(getNConds(), getNConds()), Y));
					dComplexMatrix2D.setQuick(i, getNConds(), Yij);
					if (i != getNConds())
						dComplexMatrix2D.setQuick(getNConds(), i, Yij);
				}
	...
	public void calcYPrim() {
		...
		for (int i = 0; i < yOrder; i++) {
			getYPrimSeries().setQuick(i, i, getYPrimShunt().getRealPart().getQuick(i, i) * 1e-10, getYPrimShunt().getImaginaryPart().getQuick(i, i));
		}
