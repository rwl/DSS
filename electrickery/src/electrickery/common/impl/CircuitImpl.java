...
//        doResetMeterZones();
    ...
    protected void saveBusInfo() {
        ...
        EList<Bus> savedBuses = new BasicEList<Bus>(nb);
        EList<String> savedBusNames = new BasicEList<String>(nb);
        for (int i = 0; i < nb; i++) {
            Bus bus = getBusList().get(i);
            savedBuses.add(bus);
    ...
    protected void processBusDefs() {
        String busName;
        int nNodes, np, nCond, iTerm, retVal;
        boolean nodesOK;

        CircuitElement ce = null;//getActiveCircuitElement();
        np = ce.getNPhases();
        nCond = ce.getNConds();

        int[] nodeBuffer = new int[nCond];

//        busName = ce.getBusIndex();
        ...
        for (iTerm = 0; iTerm < ce.getNTerms(); iTerm++) {
            ...
            // TODO: Parse any bus connection specified in the name.
//            parseAsBusName(nNodes, nodeBuffer);
...
//                nodeBuffer[i] = 0;
        ...
        for (int i = 0; i < getBusList().size(); i++) {
            Bus bus = getBusList().get(i);
