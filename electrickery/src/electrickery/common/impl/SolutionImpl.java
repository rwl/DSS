...
//             globals.setErrorNumber(globals.getCmdResult());
    ...
    public void setVoltageBases() {
        ...
        boolean bZoneCalc = ckt.isMeterZonesComputed();
        boolean bZoneLock = ckt.isZonesLocked();
        ckt.setMeterZonesComputed(true);
        ckt.setZonesLocked(true);

        solveZeroLoadSnapShot();

        for (Bus bus : ckt.getBuses())
            // TODO: NearestBasekV utilities method.
            bus.setKVBase(DComplexFunctions.abs.apply(getNodeV().get(bus.getRef(1))) / Math.sqrt(3.0)); // l-n base kV

//        YMatrix().initialiseNodeVBase(); // for convergence test
