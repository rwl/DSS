...
class Storage(PowerConversionElement):
    ...
    def __init__(self, kV=0.0, kW=0.0, pF=0.0, conn="Wye", kVAR=0.0, kVA=0.0, kWRated=0.0, kWhRated=0.0, kWhStored=0.0, pctStored=0.0, pctReserve=0.0, state="Idling", pctDischarge=0.0, pctCharge=0.0, pctEffCharge=0.0, pctEffDischarge=0.0, pctIdlingKW=0.0, pctIdlingKVAr=0.0, pctR=0.0, pctX=0.0, model=0, vMinPU=0.0, vMaxPU=0.0, dispMode="LoadMode", dischargeTrigger=0.0, chargeTrigger=0.0, timeChargeTrig=0.0, class=0, userModel='', userData='', debugTrace=False, bus1=None, yearly=None, daily=None, duty=None, *args, **kw_args):
        ...
        #: The present kW value.  A positive value denotes power coming OUT of the element, which is the opposite of a Load element. A negative value indicates the Storage element is in Charging state. This value is modified internally depending on the dispatch mode.
        self.kW = kW
        ...
        #: The present kVAr value.  Alternative to specifying the power factor.  Side effect:  the power factor value is altered to agree based on present value of kW.
        self.kVAR = kVAR
        ...
        #: Present operational state. In DISCHARGING mode, the Storage element acts as a generator and the kW property is positive. The element continues discharging at the scheduled output power level until the storage reaches the reserve value. Then the state reverts to IDLING. In the CHARGING state, the Storage element behaves like a Load and the kW property is negative. The element continues to charge until the max storage kWh is reached and Then switches to IDLING state. In IDLING state, the kW property shows zero. However, the resistive and reactive loss elements remain in the circuit and the power flow report will show power being consumed. Values are: "Idling", "Charging", "Discharging"
        self.state = state
        ...
        #: Equivalent percent internal resistance, ohms. Default is 0. Placed in series with internal voltage source for harmonics and dynamics modes. Use a combination of pctIdlekW and pctEffCharge and pctEffDischarge to account for losses in power flow modes.
        self.pctR = pctR

        #: Equivalent percent internal reactance, ohms. Default is 50%. Placed in series with internal voltage source for harmonics and dynamics modes. (Limits fault current to 2 pu.) Use pctIdlekVAr and kvar properties to account for any reactive power during power flow solutions.
        self.pctX = pctX

        #: Integer code for the model to use for powet output variation with voltage. Valid values are:   1: Storage element injects a CONSTANT kW at specified power factor.   2:Storage element is modeled as a CONSTANT ADMITTANCE.   3:Compute load injection from User-written Model.
        self.model = model
    ...
    # Load shape to use for duty cycle dispatch simulations such as for solar ramp rate studies. Must be previously defined as a Loadshape object. Typically would have time intervals of 1-5 seconds. If there are fewer points in the actual shape, the shape is assumed to repeat.
    duty = None
