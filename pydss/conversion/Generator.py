...
class Generator(PowerConversionElement):
    """The generator is essentially a negative load that can be dispatched.  If the dispatch value (DispValue) is 0, the generator always follows the appropriate dispatch curve, which are simply load curves. If DispValue>0 then the generator only comes on when the global circuit load multiplier exceeds DispValue.  When the generator is on, it always follows the dispatch curve appropriate for the type of solution being performed.  If you want to model a generator that is fully on whenever it is dispatched on, simply designate 'Status=Fixed'.  The default is 'Status=Variable' (i.e., it follows a dispatch curve.  You could also define a dispatch curve that is always 1.0.  Generators have their own energy meters that record:     1. Total kwh     2. Total kvarh     3. Max kW     4. Max kVA     5. Hours in operation     6. Price * kwH  Generator meters reset with the circuit energy meters and take a sample with the circuit energy meters as well. The Energy meters also used trapezoidal integration so that they are compatible with Load-Duration simulations.  Generator models are:     1. Constant P, Q  (* dispatch curve, if appropriate).     2. Constant Z  (For simple solution)     3. Constant P, |V|  like a standard power flow  [not implemented]     4. Constant P, Fixed Q  (vars)     5. Constant P, Fixed Q  (reactance)  Most of the time you will use #1 for planning studies.  The default is for the generator to be a current injection source.  Thus, its primitive Y matrix contains only the impedance that might exist from the neutral of a wye-connected generator to ground.  However, if the generator model is switched to Admittance from PowerFlow (see Set Mode command), the generator is converted to an admittance and included in the system Y matrix.  Generators are assumed balanced for the number of phases specified.  If you would like unbalanced generators, enter separate single-phase generators.
    """

    def __init__(self, bus1='', kV=0.0, kW=0.0, pf=0.0, kVAr=0.0, model="Constant_kW", vMinPU=0.0, vMaxPU=0.0, yearly='', daily='', duty='', dispMode="LoadMode", dispValue=0.0, conn="Wye", rNeut=0.0, xNeut=0.0, status="Variable", class=0, vPU=0.0, vTarget=0.0, maxKVAr=0.0, minKVAr=0.0, pvFactor=0.0, forceOn=False, kVA=0.0, MVA=0.0, xD=0.0, xDp=0.0, xDpp=0.0, h=0.0, d=0.0, userModel='', userData='', shaftModel='', shaftData='', debugTrace=False, genOn=False, shapeFactor="", forcedOn=False, fixed=False, yEq="", yEq95="", yEq105="", vBase=0.0, vBase95=0.0, vBase105=0.0, varBase=0.0, varMin=0.0, varMax=0.0, deltaQMax=0.0, dQdV=0.0, dQdVSaved=0.0, yQFixed=0.0, circuit=None, *args, **kw_args):
        ...
        #: Integer code for the model to use for generation variation with voltage. Valid values are:     1:Generator injects a constant kW at specified power factor.     2:Generator is modeled as a constant admittance.     3:Const kW, constant kV.  Somewhat like a conventional transmission     power flow P-V generator.     4:Const kW, Fixed Q (Q never varies)     5:Const kW, Fixed Q(as a constant reactance)     6:Compute load injection from User-written Model.(see usage of Xd,Xdp)     7:Constant kW, kvar, but current limited below Vminpu Values are: "Constant_kW", "Constant_Y", "Constant_kW_and_kV", "Const_kW_Fixed_Q", "Const_kW_Fixed_Q_Const_X", "User_model", "Const_kW_KVAr_Limited_I"
        self.model = model
        ...
        #: In default mode, gen is either always on or follows dispatch curve as specified.  Otherwise, the gen comes on when either the global default load level or the price level exceeds the dispatch value. Values are: "LoadMode", "PriceMode"
        ...
        #: If Fixed, then dispatch multipliers do not apply. The generator is alway at full power when it is ON. Default is Variable (follows curves). Values are: "Variable", "Fixed"
        self.status = status
