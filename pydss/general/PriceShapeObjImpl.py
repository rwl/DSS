...
from dss.general.PriceShapeObj import (PriceShapeObj,)
...
class PriceShapeObjImpl(DSSObjectImpl, PriceShapeObj):
    ...
    def __init__(self, parClass, priceShapeName):
        super(PriceShapeObjImpl, self)(parClass)
        self.setName(priceShapeName.toLowerCase())
                    ...
                    if self.numPoints == 1:
                        ...
                        while True:
                            ...
                            if self.Math.abs(self.hours[i] - hr) < 1.0000000000000002e-05:
                                ...
                                result = self.priceValues[i]
                                self.lastValueAccessed = i
                                return result
                            elif self.hours[i] > hr:
                                # interpolate for price
                                self.lastValueAccessed = i - 1
                                result = self.priceValues[self.lastValueAccessed] + (((hr - self.hours[self.lastValueAccessed]) / (self.hours[i] - self.hours[self.lastValueAccessed])) * (self.priceValues[i] - self.priceValues[self.lastValueAccessed]))
                                return result
                        # if we fall through the loop, just use last value
                        self.lastValueAccessed = self.numPoints - 1
                        result = self.priceValues[self.numPoints]
                return result
            else:
                i, = _0
                if i < self.numPoints and i >= 0:
                    self.lastValueAccessed = i
                    return self.priceValues[i]
                else:
                    return 0.0
        else:
            raise ARGERROR(1, 1)
    ...
    def calcMeanandStdDev(self):
        if self.numPoints > 0:
            if self.interval > 0.0:
                MathUtil.RCDMeanandStdDev(self.priceValues, self.numPoints, self.mean, self.stdDev)
            else:
                MathUtil.curveMeanAndStdDev(self.priceValues, self.hours, self.numPoints, self.mean, self.stdDev)
    ...
    def dumpProperties(self, f, complete):
        super(PriceShapeObjImpl, self).dumpProperties(f, complete)
        _0 = True
        i = 0
        while True:
            if _0 is True:
        ...
        while True:
            ...
            if (_3 is True) or (_2 == 2):
                ...
                while True:
                    ...
                    if not (i < self.numPoints):
                        break
                    result = result + String.format.format('%-g, ', self.priceValues[i])
                break
            ...
            if True:
                _3 = True
                result = super(PriceShapeObjImpl, self).getPropertyValue(index)
                break
            break
        _6 = index
        _7 = False
    ...
    def initPropertyValues(self, arrayOffset):
        ...
        # action option
        super(PriceShapeObjImpl, self).initPropertyValues(PriceShape.NumPropsThisClass)
    ...
    def getPriceValues(self):
        return self.priceValues

    def setPriceValues(self, values):
