...
class Relay(ControlElement):
    """A Relay is a control element that is connected to a terminal of a circuit element and controls the switches in the same or another terminal.  The control is usually placed in the terminal of a line or transformer, but it could be any element.  Voltage relay is a definite time relay that operates after the voltage stays out of bounds for a fixed time interval.  It will then reclose a set time after the voltage comes back in the normal range.
    """

    def __init__(self, monitoredObj='', monitoredTerm=0, switchedObj='', switchedTerm=0, type="Current", phaseCurve='', groundCurve='', phaseTrip=0.0, groundTrip=0.0, tDPhase=0.0, tDGround=0.0, phaseInst=0.0, groundInst=0.0, reset=0.0, shots=0, recloseIntervals=0.0, delay=0.0, kVBase=0.0, pctPickup47=0.0, pctAmps46=0.0, pctPickup46=0.0, iSQT46=0.0, variable='', overtrip=0.0, undertrip=0.0, breakerTime=0.0, action="Trip_Open", overvoltCurve=None, undervoltCurve=None, *args, **kw_args):
        ...
        #: One of a legal relay type:     Current Voltage Reversepower 46 (neg seq current)     47 (neg seq voltage)     Generic (generic over/under relay) Default is overcurrent relay     (Current) Specify the curve and pickup settings appropriate for each     type. Generic relays monitor PC Element Control variables and trip on     out of over/under range in definite time. Values are: "Current", "Fortyseven", "Generic"
        self.type = type
        ...
        #: Action that overrides the relay control. Simulates manual control on breaker. 'Trip' or 'Open' causes the controlled element to open and lock out. 'Close' causes the controlled element to close and the relay to reset to its first operation. Values are: "Trip_Open", "Close"
        self.action = action
