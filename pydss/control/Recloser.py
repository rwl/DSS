...
class Recloser(ControlElement):
    """A Recloser is a control element that is connected to a terminal of a circuit element and controls the switches in the same or another terminal.  The control is usually placed in the terminal of a line or transformer, but it could be any element  CktElement to be controlled must already exist.
    """

    def __init__(self, monitoredObj='', monitoredTerm=0, switchedObj='', switchedTerm=0, nFast=0, phaseFast='', phaseDelayed='', groundFast='', groundDelayed='', phaseTrip=0.0, groundTrip=0.0, phaseInst=0.0, groundInst=0.0, reset=0.0, shots=0, recloseIntervals=0.0, delay=0.0, action="Trip_Open", tDPhFast=0.0, tDGrFast=0.0, tDPhDelayed=0.0, tDGrDelayed=0.0, *args, **kw_args):
        ...
        #: Name of the TCC Curve object that determines the Ground Delayed trip.  Must have been previously defined as a TCC_Curve object. Multiplying the current values in the curve by the 'groundtrip' value gives the actual current.
        ...
        #: Action that overrides the Recloser control.  Simulates manual control on recloser 'Trip' or 'Open' causes the controlled element to open and lock out. 'Close' causes the controlled element to close and the Recloser to reset to its first operation. Values are: "Trip_Open", "Close"
        self.action = action
