...
public class KLUSolve {

	private static boolean SYMMETRIC_MATRIX = true;

	private static Map<UUID, KLUSystem> sysMap = new HashMap<UUID, KLUSystem>();
	...
	public static int solveSparseSet(UUID uuid, Complex[] x, int x_offset, Complex[] b, int b_offset) {
		DZcsa acxX, acxB;
		KLUSystem sys = sysMap.get(uuid);

		if (sys != null) {
			if (sys.bFactored == false) {
				sys.factorSystem();
			}
			if (sys.bFactored) {
	...
	public static int deleteSparseSet(UUID uuid) {
		KLUSystem sys = sysMap.remove(uuid);
	...
	public static int addPrimitiveMatrix(UUID uuid, int nOrder, int[] nodes, Complex[] mat) {
	 ...
	 */
	public static int addPrimitiveMatrix(UUID uuid, int nOrder, int[] nodes, int node_offset, Complex[] mat, int m_offset) {
		int rc = 0;
		KLUSystem sys = sysMap.get(uuid);
